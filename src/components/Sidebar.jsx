...
const Sidebar = ({ activeMenu, setActiveMenu }) => {
    ...
    const activeLink = 'flex items-center gap-5 pl-4 pt-3 pb-2.5 rounded-lg text-white bg-gray-900 text-md m-2'
    const normalLink = `flex items-center gap-5 pl-4 pt-3 pb-2.5 rounded-lg 
    text-gray-700 text-md dark:text-gray-200 dark:hover:text-black hover:bg-light-gray m-2`
    ...
    return (
        ...
        md:hover:overflow-auto pb-10'>
            {activeMenu && (<>
                ...
                <div className='mt-10'>
                    {links.map((item, __i) => (
                        <div key={`${item.title}-${__i}`}>
                            <p className="text-gray-400 m-3 mt-4 uppercase">
                                {item.title}
                            </p>
                            {item.links.map((link, __n) => (
                                <NavLink
                                    key={`${link.name}-${__n}`}
                                    to={`/${link.name}`}
                                    onClick={() => { }}
                                ...
                                >
                                    {link.icon} <sapn className="capitalize">{link.name} </sapn>
                                </NavLink>
                            ))
                            }
                        </div>
